...
public class CommandLoop {
    ...
    public CommandLoop(AuthorizationService auth,
        ...
        switch (cmd) {
            ...
            case "budget" -> {
                ...
                if ("set".equals(sub)) {
                    ...
                    Wallet w = currentWallet();
                    Map<String, StatsService.BudgetStatus> st = new org.com.financeApp.services.StatsService().budgetStatus(w);
                        ...
                        st.entrySet().stream()
                                ...
                                .forEach(e -> {
                                    ...
                                    System.out.println("- " + e.getKey()
                                            + ": " + bs.limit()
                                            + ", Оставшийся бюджет: " + bs.remaining()
            ...
            case "income", "expense" -> {
                ...
                String sub = tokens[1].toLowerCase();
                if (!"add".equals(sub)) {
                    throw new IllegalArgumentException(cmd + " add <category> <amount> [YYYY-MM-DD] [comment...]");
                }
                ...
                if ("income".equals(cmd)) {
                    warnings = walletService.addIncome(auth.getCurrentUser(), category, amount, date, comment);
                } else {
                    warnings = walletService.addExpense(auth.getCurrentUser(), category, amount, date, comment);
                }
            ...
            case "stats" -> {
                ...
                String sub = tokens[1].toLowerCase();
                if (!"show".equals(sub)) throw new IllegalArgumentException("stats show [--from YYYY-MM-DD] [--to YYYY-MM-DD]");
                ...
                Wallet w = currentWallet();
                String report = (from == null && to == null)
            ...
            case "report" -> {
                ...
                for (int i = 3; i < tokens.length; i++) {
                    if ("--from".equals(tokens[i]) && i + 1 < tokens.length) {
                        ...
                        throw new IllegalArgumentException("report file <path> [--from YYYY-MM-DD] [--to YYYY-MM-DD]");
                    }
                }

...
class WalletFileStorageTest {
    ...
    void save_then_loadOrCreate_shouldRestoreWallet() throws Exception {
        ...
        w.addTransaction(new Transaction(TransactionType.INCOME, "Зарплата", 20000, LocalDate.of(2025, 12, 1), "pay"));
        w.addTransaction(new Transaction(TransactionType.EXPENSE, "Еда", 500, LocalDate.of(2025, 12, 2), "coffee"));
    ...
    void export_then_import_snapshot_shouldRestoreState() throws Exception {
        ...
        w.addCategory("Еда");
        w.setBudget("Еда", 100);
        w.addTransaction(new Transaction(TransactionType.EXPENSE, "Еда", 80, LocalDate.of(2025,12,1), null));
